...
const ALIAS_TO_CANONICAL = {
  ...
  'Selesai (LHU Disahkan)': FPPL_STATUSES.SELESAI,
  'LHU Disahkan': FPPL_STATUSES.MENUNGGU_PENGAMBILAN_LHU,
...
export function getFpplStatusDisplayLabel(status) {
  return normalizeFpplStatus(status);
}
...
export function getFpplStatusBadgeClass(status) {
  ...
  const styleMap = {
    ...
    [FPPL_STATUSES.MENUNGGU_VERIFIKASI_PEMBAYARAN]: { bg: 'bg-yellow-100', text: 'text-yellow-700' },
    ...
    [FPPL_STATUSES.MENUNGGU_PENGAMBILAN_LHU]: { bg: 'bg-teal-100', text: 'text-teal-700' },
