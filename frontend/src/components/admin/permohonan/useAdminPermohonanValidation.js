...
export function useAdminPermohonanValidation({
  ...
  const handleVerifyManualPayment = async (action) => {
    if (!selectedRequest) return;

    const normalizedAction = action === 'approve' ? 'approve' : 'reject';
    const cleanNote = cleanAdminNote(paymentVerificationNote);

    if (normalizedAction === 'reject' && !cleanNote) {
      showWarning('Catatan penolakan pembayaran wajib diisi.');
      return;
    }

    setSaving(true);
    try {
      const data = await adminPermohonanApi.verifyPayment(selectedRequest.id_registrasi, {
      ...
      if (data.success) {
        ...
        showSuccess(
          normalizedAction === 'approve'
            ...
            : 'Pembayaran ditolak. Pelanggan dapat upload ulang bukti pembayaran.'
        );
      } else {
        showError(data.message || 'Gagal memverifikasi pembayaran.');
      }
    } catch (error) {
