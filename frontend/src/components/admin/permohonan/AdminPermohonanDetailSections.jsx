...
export function SampleParameterDetailSection({
  ...
  return (
    <div ref={sectionRef} className="bg-white rounded-xl shadow-sm border border-gray-100 mb-4 transition-all">
      ...
      {expandedSection === 'info' && (
        <div className="px-6 pb-6">
          {getRequestSamples(selectedRequest).length > 0 ? (
            <div className="space-y-6">
              {getRequestSamples(selectedRequest).map((requestSample, idx) => {
                          ...
                          >
                            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-4">
                              ...
                              <div className="text-sm text-gray-600 space-y-1">
                                <div>
                                  Tanggal Pengambilan : {formatDate(actualSample.tanggal_pengambilan_sampel || actualSample.tanggalPengambilanSampel)}
                                </div>
                                <div>
                                  Tanggal Penerimaan : {formatDateTime(getSampleReceiptDate(actualSample), getSampleReceiptTime(actualSample))}
                                </div>
                              </div>
                            </div>
...
export function FpplDocumentSection({
  selectedRequest,
  expandedSection,
  toggleSection,
  openGeneratedFile,
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 mb-4 transition-all">
      <button
        onClick={() => toggleSection('dokumen-fppl')}
        className="w-full flex items-center justify-between p-6 text-left"
      >
        <div className="flex items-center gap-3">
          <FileText className="w-6 h-6 text-emerald-600" />
          <h2 className="text-xl font-semibold text-gray-900">Dokumen FPPL</h2>
        </div>
        {expandedSection === 'dokumen-fppl' ? <ChevronUp className="w-5 h-5 text-gray-500" /> : <ChevronDown className="w-5 h-5 text-gray-500" />}
      </button>
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-700">
                  Dokumen FPPL dibuat otomatis setelah pembayaran valid dan jadwal sampel sudah disetujui pelanggan/admin.
                </p>
                <p className="mt-2 text-sm text-gray-700">
                  Nomor FPPL: <span className="font-semibold text-gray-900">{nomorFppl}</span>
                </p>
                {!filePath && (
                  <p className="mt-2 text-sm text-amber-700">
                    File FPPL belum tersedia. Selesaikan syarat pembayaran dan jadwal, lalu refresh data permohonan.
                  </p>
                )}
              </div>
