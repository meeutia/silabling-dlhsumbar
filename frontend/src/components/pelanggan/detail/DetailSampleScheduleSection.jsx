...
export function DetailSampleScheduleSection({
  ...
  const isScheduleApproved = (schedule, jenisJadwal) => {
    ...
    if (jenisJadwal === 'LHU' && status === 'Dijadwalkan') return true;
    if (['Disetujui Pelanggan', 'Disetujui Admin', 'Disetujui'].includes(status)) return true;
    return Boolean(getApprovedScheduleChange(jenisJadwal));
  };

  const getApprovedByLabel = (schedule, jenisJadwal) => {
    const status = String(getScheduleApprovalStatus(schedule, jenisJadwal)).trim();
    if ((jenisJadwal === 'LHU' && status === 'Dijadwalkan') || status === 'Disetujui Admin' || (!status && getApprovedScheduleChange(jenisJadwal))) return 'admin';
  ...
  const canRequestSampleReschedule = Boolean(activeSchedule) && !isSampleReceived && !isSampleScheduleApproved;
  const canRequestLhuReschedule = hasLhuPickupSchedule && lhuPickupStatus !== 'Sudah Diambil' && !isLhuScheduleApproved;
      ...
      <form onSubmit={handleScheduleChangeSubmit} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-gray-100 bg-white p-6 shadow-2xl">
        <div className="mb-5 flex items-start justify-between gap-4">
          ...
          <button
            ...
            aria-label="Tutup modal perubahan jadwal"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
