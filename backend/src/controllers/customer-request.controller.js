...
const createRequest = async (req, res) => {
  try {
    ...
    notificationService.notifyAdminPermohonanBaru({
...
const processPaymentDecision = async (req, res) => {
  try {
    ...
    if (action === 'approve') {
      ...
      return successResponse(res, 'Persetujuan berhasil disimpan. Silakan upload bukti pembayaran manual.', data);
    }
...
const confirmPaymentSubmitted = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await PaymentService.confirmPaymentSubmitted(id, req.user.nik, req.file || null);

    notificationService.notifyPaymentProofSubmittedToAdmin({
      idRegistrasi: data?.id_registrasi || data?.idRegistrasi || id,
    }).catch((notifyError) => {
      console.error('notifyPaymentProofSubmittedToAdmin confirmPaymentSubmitted error:', notifyError);
    });

    return successResponse(res, 'Bukti pembayaran berhasil dikirim. Menunggu verifikasi admin.', data);
  } catch (error) {
    console.error('confirmPaymentSubmitted error:', error);
    const code = error.message === 'FORBIDDEN' ? 403 : 400;
    const message = error.message === 'FORBIDDEN'
      ? 'Anda tidak memiliki akses ke permohonan ini.'
