...
const FpplDocumentService = require('../services/fppl/fppl-document.service');
...
const verifyManualPayment = async (req, res) => {
  try {
    const { id } = req.params;
    const { action, note, catatan } = req.body || {};
    const finalNote = note || catatan || null;

    const data = await PaymentService.verifyPaymentByAdmin(id, req.user.nik, action, finalNote);

    const isApproved = String(action || '').toLowerCase() === 'approve';
    const message = isApproved
      ? 'Pembayaran berhasil diverifikasi. Permohonan dilanjutkan ke tahap sampel.'
      : 'Pembayaran ditolak. Pelanggan dapat mengupload ulang bukti pembayaran.';

    try {
      await notificationService.notifyRequestStatusChanged({
        idRegistrasi: data?.id_registrasi || id,
        statusTerbaru: data?.status,
        catatanPetugas: finalNote,
      });
    } catch (notifyError) {
      console.error('notifyRequestStatusChanged verifyManualPayment error:', notifyError);
    }

    return successResponse(res, message, data);
  } catch (error) {
    console.error('verifyManualPayment error:', error);
    return errorResponse(res, error.message || 'Gagal memverifikasi pembayaran.', 400);
  }
};
...
const generateFpplDocument = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await FpplDocumentService.saveFpplPdf(id, req.user?.nik || null);

    return successResponse(res, 'Dokumen FPPL berhasil digenerate.', data);
  } catch (error) {
    console.error('generateFpplDocument error:', error);
    return errorResponse(res, error.message || 'Gagal generate dokumen FPPL.', 400);
  }
};

const downloadFpplDocument = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await FpplDocumentService.saveFpplPdf(id, req.user?.nik || null);
    const filePath = FpplDocumentService.resolveFpplPublicPath(result.file_fppl);

    if (!filePath) {
      return errorResponse(res, 'Path dokumen FPPL tidak valid.', 400);
    }

    return res.download(filePath, result.filename || `fppl-${id}.pdf`);
  } catch (error) {
    console.error('downloadFpplDocument error:', error);
    return errorResponse(res, error.message || 'Gagal membuka dokumen FPPL.', 400);
  }
};
...
module.exports = {
  ...
  markDeferredPayment,
  verifyManualPayment,
  generateFpplDocument,
  downloadFpplDocument,
