...
const SAMPLE_APPROVED_STATUSES = [SAMPLE_STATUS_APPROVED_CUSTOMER, SAMPLE_STATUS_APPROVED_ADMIN];
const LHU_APPROVED_STATUSES = [LHU_STATUS_APPROVED_CUSTOMER, LHU_STATUS_APPROVED_ADMIN];

async function sendScheduleChangeNotification(label, callback) {
  try {
    await callback();
  } catch (error) {
...
function isCompletedRequestStatus(status) {
  return normalizeText(status) === RequestStatus.COMPLETED;
}

function isSampleScheduleClosed(schedule = {}) {
  return normalizeText(schedule?.status_jadwal) === SAMPLE_STATUS_DONE;
}

function isLhuScheduleClosed(schedule = {}) {
  return WorkflowGuard.isLhuPickedUp(schedule);
}

async function autoRejectClosedScheduleChange(row, transaction, actorNik = null) {
...
function normalizeScheduleKind(value) {
  ...
  if (['SAMPEL', 'SAMPLE', 'SAMPLING'].includes(text)) return 'SAMPEL';
  if (['LHU', 'PENGAMBILAN_LHU', 'LHU_PICKUP'].includes(text)) return 'LHU';
  return '';
}

async function loadHolidaysForScheduleValidation() {
  try {
...
async function validateScheduleDateOrThrow(value, label = 'Tanggal usulan') {
  const holidays = await loadHolidaysForScheduleValidation();
  return assertScheduleBusinessDateOrThrow(value, label, holidays);
}

async function assertCustomerOwnsRequest(idRegistrasi, currentNik, transaction) {
...
async function findActiveSchedule({ idRegistrasi, jenisJadwal, transaction, lock = false }) {
...
async function confirmScheduleApproval(payload = {}, currentNik) {
  const requestId = String(payload.idRegistrasi || payload.id_registrasi || '').trim();
  const jenisJadwal = normalizeScheduleKind(payload.jenisJadwal || payload.jenis_jadwal || payload.type);
  ...
  const approvedSchedule = await sequelize.transaction(async (transaction) => {
    await assertCustomerOwnsRequest(requestId, userNik, transaction);
    ...
    const updated = await schedule.update(
      { status_pengambilan: LHU_STATUS_APPROVED_CUSTOMER },
      { transaction }
    );
  ...
  if (jenisJadwal === 'SAMPEL') {
    const fpplDocument = await FpplDocumentService.tryGenerateFpplPdfIfReady(requestId, { actorNik: userNik });
    if (fpplDocument?.file_fppl) {
      return { ...approvedSchedule, file_fppl: fpplDocument.file_fppl, fileFppl: fpplDocument.file_fppl };
    }
  }
...
async function createScheduleChangeRequest(payload = {}, currentNik) {
  const requestId = String(payload.idRegistrasi || payload.id_registrasi || '').trim();
  const jenisJadwal = normalizeScheduleKind(payload.jenisJadwal || payload.jenis_jadwal || payload.type);
  ...
  tanggalUsulan = await validateScheduleDateOrThrow(tanggalUsulan, 'Tanggal usulan');
  ...
  const created = await sequelize.transaction(async (transaction) => {
    ...
    const idPengajuan = await generateId(
      PengajuanPerubahanJadwal,
      'id_pengajuan_jadwal',
      'PJG-',
      transaction,
    ...
    return getPlain(created);
  });

  await sendScheduleChangeNotification('pengajuan perubahan jadwal ke admin', () =>
    notificationService.notifyScheduleChangeSubmittedToAdmin({
...
async function listScheduleChangeRequests({ status, jenisJadwal } = {}) {
  const where = {};
  const normalizedKind = normalizeScheduleKind(jenisJadwal);
  if (normalizedKind) where.jenis_jadwal = normalizedKind;
  if (status) where.status_pengajuan = String(status).trim();
  ...
  return rows
    .map(getPlain)
    .filter((row) => {
      if (row.status_pengajuan !== STATUS_PENDING && row.statusPengajuan !== STATUS_PENDING) return true;
      if (isCompletedRequestStatus(row.fppl?.status_fppl || row.Fppl?.status_fppl)) return false;

      if (row.jenis_jadwal === 'SAMPEL') {
        const schedule = row.jadwal_sampel || row.jadwalSampel || row.JadwalSampel;
        return !isSampleScheduleClosed(schedule);
      }

      if (row.jenis_jadwal === 'LHU') {
        const schedule = row.jadwal_pengambilan_lhu || row.jadwalPengambilanLhu || row.JadwalPengambilanLhu;
        return !isLhuScheduleClosed(schedule);
      }
...
async function decideScheduleChangeRequest(idPengajuan, payload = {}, currentNik) {
  ...
  const decided = await sequelize.transaction(async (transaction) => {
    ...
    if (isCompletedRequestStatus(request.status_fppl)) {
      return autoRejectClosedScheduleChange(row, transaction, userNik);
    }

    const now = new Date();

    if (action === 'approve') {
      await validateScheduleDateOrThrow(row.tanggal_usulan, 'Tanggal usulan');
    ...
    if (row.jenis_jadwal === 'SAMPEL') {
      ...
      if (isSampleScheduleClosed(schedule)) {
        return autoRejectClosedScheduleChange(row, transaction, userNik);
      }
      ...
      if (isLhuScheduleClosed(schedule)) {
        return autoRejectClosedScheduleChange(row, transaction, userNik);
      }
  ...
  if (action === 'approve') {
    await sendScheduleChangeNotification('persetujuan perubahan jadwal ke pelanggan', () =>
      notificationService.notifyScheduleChangeApprovedToCustomer({
        idPengajuanJadwal: decided.id_pengajuan_jadwal,
      })
    );
  } else {
    await sendScheduleChangeNotification('penolakan perubahan jadwal ke pelanggan', () =>
      notificationService.notifyScheduleChangeRejectedToCustomer({
        idPengajuanJadwal: decided.id_pengajuan_jadwal,
      })
    );
  }

  if (action === 'approve' && decided?.jenis_jadwal === 'SAMPEL') {
    const fpplDocument = await FpplDocumentService.tryGenerateFpplPdfIfReady(decided.id_registrasi, { actorNik: userNik });
    if (fpplDocument?.file_fppl) {
      return { ...decided, file_fppl: fpplDocument.file_fppl, fileFppl: fpplDocument.file_fppl };
    }
  }
...
async function cancelScheduleChangeRequest(idPengajuan, currentNik, role) {
...
module.exports = {
  ...
  STATUS_REJECTED,
  STATUS_CANCELLED,
  SAMPLE_STATUS_SCHEDULED,
  SAMPLE_STATUS_APPROVED_CUSTOMER,
  SAMPLE_STATUS_APPROVED_ADMIN,
  LHU_STATUS_SCHEDULED,
  LHU_STATUS_APPROVED_CUSTOMER,
  LHU_STATUS_APPROVED_ADMIN,
