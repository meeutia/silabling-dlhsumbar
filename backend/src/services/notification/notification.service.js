...
function mapRevisionItemsForEmail(revisions = [], details = []) {
  ...
  return (rows || []).map((row, index) => {
    const detailId = safeString(row.id_penugasan_detail || row.idPenugasanDetail || row.hasilTargetKey || '').trim();
    const detail = detailById.get(detailId) || row || {};
    ...
    return {
      id_penugasan_detail: detailId || detail.id_penugasan_detail || null,
      nama_parameter:
