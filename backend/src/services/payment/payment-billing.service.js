...
const buildSamplingLabel = (requestJson) => {
...
const getPlainPayment = (value) => {
  if (!value) return null;
  if (typeof value.get === 'function') return value.get({ plain: true });
  return value;
};

const collectPaymentRowsFromInvoice = (invoice = {}) => {
  // invoice bisa bernilai null saat permohonan belum punya invoice.
  // Default parameter hanya menangani undefined, bukan null.
  // Karena itu perlu dinormalisasi dulu agar detail permohonan tidak crash.
  const safeInvoice = invoice || {};

  const candidates = [
    safeInvoice.payments,
    safeInvoice.Payments,
    safeInvoice.payment,
    safeInvoice.Payment,
    safeInvoice.pembayaran,
    safeInvoice.Pembayaran,
    safeInvoice.paymentData,
  ];
...
const buildPaymentSummaryPayload = ({ latestPayment, paymentMethod, totalTagihan }) => {
  if (!latestPayment) return null;

  const methodCode = paymentMethod?.code || latestPayment.metode_bayar || null;
  const isDeferredByAdmin =
    methodCode === INTERNAL_PAYMENT_METHOD.code ||
    ['MANUAL', 'PEMBAYARAN_AKHIR_ADMIN', 'BAYAR_NANTI_ADMIN'].includes(String(latestPayment.metode_bayar || '').toUpperCase());
  ...
  return {
    ...
    method_label: paymentMethod?.label || latestPayment.metode_bayar || '-',
    metodeBayar: latestPayment.metode_bayar || null,
    metode_bayar: latestPayment.metode_bayar || null,

    amount: Number(latestPayment.jumlah_bayar || totalTagihan || 0),
    jumlahBayar: Number(latestPayment.jumlah_bayar || totalTagihan || 0),
    jumlah_bayar: Number(latestPayment.jumlah_bayar || totalTagihan || 0),

    paidAt: latestPayment.paid_at || null,
    paid_at: latestPayment.paid_at || null,

    isDeferredByAdmin,
    is_deferred_by_admin: isDeferredByAdmin,

    statusVerifikasi: latestPayment.status_verifikasi || null,
    status_verifikasi: latestPayment.status_verifikasi || null,

    buktiBayarPath: latestPayment.bukti_bayar_path || null,
    bukti_bayar_path: latestPayment.bukti_bayar_path || null,

    verifiedBy: latestPayment.verified_by || null,
    verified_by: latestPayment.verified_by || null,
    verifiedAt: latestPayment.verified_at || null,
    verified_at: latestPayment.verified_at || null,

    catatanVerifikasi: latestPayment.catatan_verifikasi || null,
    catatan_verifikasi: latestPayment.catatan_verifikasi || null,
    adminNote: latestPayment.catatan_verifikasi || null,

    gateway: null,
  };
};

const loadRequestForBilling = async (requestId, transaction = undefined) => {
...
const createOrRefreshInvoiceForRequest = async (requestId, transaction = undefined) => {
  const requestRecord = await loadRequestForBilling(requestId, transaction);
  if (!requestRecord) {
  ...
  for (const sample of sampleRows) {
    ...
    const jumlahSampel =
      Number(sample.jumlah_sampel || sample.jumlahSampel || 1) || 1;
    ...
    for (const fpm of fpmRows) {
      ...
      const isSubkontrak =
        Number(parameterMetode?.is_subkontrak) === 1 ||
  ...
  const subtotalUji = parameterItems.reduce(
    (sum, item) => sum + item.subtotal,
    0
  );

  const subtotalPengambilan = normalizeAmount(
    requestJson.tarif_pengambilan?.tarif || requestJson.TarifPengambilan?.tarif || 0
  );
...
const buildInvoiceSummary = async (requestId, transaction = undefined) => {
  const requestRecord = await loadRequestForBilling(requestId, transaction);
  if (!requestRecord) {
    return null;
  }
  ...
  const subtotalPengambilan = latestInvoice
    ...
    : normalizeAmount(
        requestJson.tarif_pengambilan?.tarif ||
  ...
  const paymentRows = collectPaymentRowsFromInvoice(latestInvoice);
  ...
  const tanggalInvoiceDateOnly = toDateOnlyString(latestInvoice?.tanggal_invoice);
  ...
  return {
    ...
    rincian: {
      ...
      metodeSampling: buildSamplingLabel(requestJson),
      metode_sampling: buildSamplingLabel(requestJson),
    ...
    Payment: paymentPayload,
    latestPayment: paymentPayload,
    latest_payment: paymentPayload,

    // Alias root-level supaya frontend admin tetap bisa membaca bukti pembayaran
    // meskipun payload invoice lama tidak membawa object payment secara lengkap.
    buktiBayarPath: paymentPayload?.buktiBayarPath || null,
    bukti_bayar_path: paymentPayload?.bukti_bayar_path || null,
    statusVerifikasiPembayaran: paymentPayload?.statusVerifikasi || null,
    status_verifikasi_pembayaran: paymentPayload?.status_verifikasi || null,
    jumlahBayar: paymentPayload?.jumlahBayar || null,
    jumlah_bayar: paymentPayload?.jumlah_bayar || null,
  };
};


const ensureCustomerOwnsRequest = async (requestId, userNik, transaction = undefined) => {
...
// Kompatibilitas lama: kolom status_keputusan_permohonan tetap ada di DB,
// tetapi bukan lagi sumber kebenaran keputusan pelanggan.
// Sumber kebenaran keputusan pelanggan adalah fppl.status_fppl.


const updateCustomerApprovalStatus = async (requestId, status, transaction = undefined) => {
