...
function getStaffRoleOrderValue(row = {}) {
  ...
  const roleIndex = STAFF_ROLE_ORDER.indexOf(idRole);
  if (roleIndex >= 0) return roleIndex;

  const mappedRoleId = STAFF_ROLE_LABEL_TO_ID[normalizeText(row.role)];
  const mappedRoleIndex = STAFF_ROLE_ORDER.indexOf(mappedRoleId);
  if (mappedRoleIndex >= 0) return mappedRoleIndex;

  if (!row.nik || !row.username) return STAFF_WITHOUT_ACCOUNT_ORDER;

  return STAFF_ROLE_FALLBACK_ORDER;
}
...
function validateUsername(username) {
  ...
  if (!value) throw new Error('Username wajib diisi.');
  if (/\s/.test(value)) throw new Error('Username tidak boleh mengandung spasi.');
  return value;
}
...
function mapStaffRow(row) {
  ...
  return {
    ...
    role_order: getStaffRoleOrderValue(row),
    roleOrder: getStaffRoleOrderValue(row),
...
async function listStaff(query = {}) {
  ...
  if (search) {
    rows = rows.filter((row) =>
      [row.nik, row.username, row.email, row.nama_pegawai, row.jabatan, row.nip, row.no_wa, row.role]
        .filter(Boolean)
