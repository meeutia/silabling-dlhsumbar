...
function formatLocalYmd(dateValue = new Date()) {
...
function formatLocalHms(dateValue = new Date()) {
  const date = dateValue instanceof Date ? dateValue : new Date(dateValue);
...
function isPetugasSampling(value) {
  return String(value || '').toLowerCase() === 'petugas';
}

function resolveTanggalPengambilanSampel({ itemPayload = {}, payload = {}, request = {}, jadwal = null }) {
  const explicitDate =
    itemPayload.tanggal_pengambilan_sampel ||
    itemPayload.tanggalPengambilanSampel ||
    payload.tanggal_pengambilan_sampel ||
    payload.tanggalPengambilanSampel ||
    null;

  if (explicitDate) return normalizeDateOnly(explicitDate);
  if (isPetugasSampling(request.jenis_pengambilan_sampel)) return normalizeDateOnly(jadwal?.tanggal_jadwal);

  return null;
}

const generateSampleAbbreviation = (jenisSampelName, idJenisSampel = null) => {
  ...
  if (sampleAbbreviationById[normalizedId]) return sampleAbbreviationById[normalizedId];
  if (!jenisSampelName) return 'SMP';
...
const normalizeSampleCondition = (condition = '') => {
  const value = String(condition).trim().toLowerCase();

  if (value === 'tidak sesuai' || value === 'rusak' || value === 'rejected' || value === 'not suitable') return 'Tidak Sesuai';
  return 'Sesuai';
};

const buildNoSampel = (sequenceNumber, jenisSampelName, dateValue = new Date(), idJenisSampel = null) => {
  const date = dateValue ? new Date(dateValue) : new Date();
  const month = date.getMonth() + 1;
  const year = date.getFullYear();

  const romanMonth = monthToRoman(month);
  const sampleCode = generateSampleAbbreviation(jenisSampelName, idJenisSampel);

  return `${sequenceNumber}/${sampleCode}/${romanMonth}/${year}`;
};

const getNextSampleSequence = async (transaction) => {
...
const getLatestInvoiceForSampleReceipt = async (idRegistrasi, transaction) => {
...
const isSettledInvoiceStatus = (status) => ['Lunas', 'Bayar Nanti'].includes(String(status || '').trim());

const normalizeLegacyPaymentVerificationIfSettled = async (request, transaction) => {
  if (request.status_fppl !== RequestStatus.WAITING_PAYMENT_VERIFICATION) {
    return { invoice: null, normalized: false };
  }

  const invoice = await getLatestInvoiceForSampleReceipt(request.id_registrasi, transaction);

  if (invoice && isSettledInvoiceStatus(invoice.status_invoice)) {
    await request.update({ status_fppl: RequestStatus.WAITING_SAMPLE }, { transaction });
    request.status_fppl = RequestStatus.WAITING_SAMPLE;
...
const assertRequestReadyForSampleReceipt = async (request, transaction) => {
  const legacyNormalization = await normalizeLegacyPaymentVerificationIfSettled(request, transaction);

  if (request.status_fppl !== RequestStatus.WAITING_SAMPLE) {
    ...
    throw new Error(`Sampel hanya dapat diterima saat status permohonan "${RequestStatus.WAITING_SAMPLE}". Status saat ini: ${request.status_fppl}`);
  }

  const invoice = legacyNormalization.invoice || await getLatestInvoiceForSampleReceipt(request.id_registrasi, transaction);
  if (!invoice) {
    throw new Error('Invoice belum tersedia. Sampel belum dapat diterima.');
  }

  const invoiceStatus = String(invoice.status_invoice || '').trim();
  if (isSettledInvoiceStatus(invoiceStatus)) {
    return invoice;
  }
