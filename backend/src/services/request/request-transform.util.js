...
const isActiveLhuPickupSchedule = (schedule) => {
  ...
  const status = String(schedule.status_pengambilan || schedule.statusPengambilan || '').trim();
...
const deriveCustomerHistoryStatus = (requestJson = {}) => {
  ...
  if (isActiveLhuPickupSchedule(getRequestLhuPickupSchedule(requestJson))) {
...
const deriveCustomerDecisionStatus = (statusFppl) => {
  ...
  if ([RequestStatus.WAITING_SAMPLE, RequestStatus.TESTING_PROCESS, RequestStatus.COMPLETED].includes(statusFppl)) return 'Disetujui';
  return 'Menunggu';
};
