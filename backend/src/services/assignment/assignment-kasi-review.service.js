...
} = require('./assignment-kasi-revision-log.service');
...
} = require('./assignment-kasi-review-query.service');

async function getKasiReviewQueue() {
  ...
  for (const sampleInstance of sampleInstances) {
    ...
    const derivedQueueStatus = (completedRows || []).some((row) =>
      row.statusReviewHasil === SAMPLE_REVIEW_STATUS.REVISION_KASI_PENGUJIAN
    )
      ? SAMPLE_REVIEW_STATUS.REVISION_KASI_PENGUJIAN
      : SAMPLE_REVIEW_STATUS.WAIT_KASI_PENGUJIAN;
  ...
  return mappedRows.sort((a, b) =>
    String(b.idRegistrasi || '').localeCompare(String(a.idRegistrasi || '')) ||
...
async function getKasiReviewHistory() {
  const sampleInstances = await Sampel.findAll({
    include: [
      {
        model: FpplSampel,
      ...
      },
      {
        ...
        required: false,
        through: { attributes: [] },
        include: [{ model: PktBm, required: false }],
      },
    ],
  ...
  for (const sampleInstance of sampleInstances) {
    ...
    if (totalParameter <= 0) continue;

    const approvedKasiRows = resultRows.filter(
      (row) => row.statusReviewHasil === SAMPLE_REVIEW_STATUS.APPROVED_KASI_PENGUJIAN ||
        row.statusReviewHasil === LKA_HASIL_STATUS.APPROVED_KASI
    );

    if (approvedKasiRows.length !== totalParameter) continue;

    const sampleHeader = mapKasiReviewSampleHeader({
      ...sample,
      statusReviewHasil: SAMPLE_REVIEW_STATUS.APPROVED_KASI_PENGUJIAN,
      kasiPengujianReviewAt:
    ...
    const pktBm = pickObject(lhu, ['pkt_bm', 'PktBm']) || {};
  ...
  return rows.sort((a, b) =>
    String(b.kasiPengujianReviewAt || '').localeCompare(String(a.kasiPengujianReviewAt || '')) ||
...
async function getKasiReviewDetail(noSampel) {
...
async function assertSampleReadyForKasiReview(noSampel, transaction = null) {
...
async function approveKasiReview(noSampel, currentNik) {
  const sampleNo = String(noSampel || '').trim();
  ...
  return sequelize.transaction(async (transaction) => {
    ...
    const affectedLkaCodes = Array.from(
      new Set(completedRows.map((row) => row.kode_lka).filter(Boolean))
    );
...
async function reviseKasiReview(noSampel, catatanRevisi, currentNik, hasilTargets = [], revisionsPayload = null) {
  const sampleNo = String(noSampel || '').trim();
  const userNik = String(currentNik || '').trim();
  ...
  return sequelize.transaction(async (transaction) => {
    ...
    const targetDetailIds = Array.from(
      new Set(targetRows.map((row) => row.id_penugasan_detail).filter(Boolean))
    );
      ...
      await LkaHasil.update(
        {
          ...
          // Catatan revisinya hanya disimpan di lka_revisi_item sampai Penyelia menyetujui.
    ...
    // Revisi dari Kasi Pengujian disimpan di lka_revisi/lka_revisi_item.
      ...
      await createLkaRevisionLog(
        {
          ...
          items: rows.map((row) => ({
            ...
            statusItemRevisi: 'Menunggu Review Penyelia',
            status_item_revisi: 'Menunggu Review Penyelia',
          })),
        },
        transaction
      );
...
async function getPendingKasiRevisionRequests() {
...
async function reviewKasiRevisionRequest(idRevisiLka, payload = {}, penyeliaNik) {
  ...
  return sequelize.transaction(async (transaction) => {
    ...
    const catatanRevisiGabungan = Array.from(
      new Set(notificationItems.map((item) => String(item.catatan_revisi || '').trim()).filter(Boolean))
    ).join('\n');

    await LkaRevisiItem.update(
      {
        status_item_revisi: isApprove ? 'Disetujui untuk Analis' : 'Ditolak Penyelia',
        updated_at: new Date(),
      },
      {
        where: { id_revisi_lka: revisionId },
    ...
    if (hasilTargetKeys.length > 0) {
      if (isApprove) {
        ...
        await LkaHasil.update(
          { statusReviewHasil: LKA_HASIL_STATUS.REVISION },
          { where: lkaHasilWhereFromKeys(hasilTargetKeys, revision.kode_lka), transaction }
        );
      } else {
        // Jika Penyelia menolak, hasil dikembalikan ke antrean review Kasi.
        await LkaHasil.update(
          { statusReviewHasil: LKA_HASIL_STATUS.WAIT_KASI },
          { where: lkaHasilWhereFromKeys(hasilTargetKeys, revision.kode_lka), transaction }
        );

        if (sampleNos.length > 0) {
          await Sampel.update(
            { statusReviewHasil: SAMPLE_REVIEW_STATUS.WAIT_KASI_PENGUJIAN },
            { where: { no_sampel: { [Op.in]: sampleNos } }, transaction }
          );
        }
      }
