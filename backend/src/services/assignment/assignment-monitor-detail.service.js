...
async function getAssignmentDetailsByPenugasan(idPenugasan) {
  const headerInstance = await Penugasan.findByPk(idPenugasan, {
  ...
  const details = detailInstances.map((instance) => {
    ...
    const idJenisSampel =
      sampleRows.find((sample) => sample.id_jenis_sampel)?.id_jenis_sampel ||
      null;
