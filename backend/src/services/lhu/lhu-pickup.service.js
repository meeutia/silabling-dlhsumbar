...
function normalizePickupTimeForDb(value) {
...
function isActivePickupScheduleStatus(status) {
  return ['Dijadwalkan', 'Disetujui Pelanggan', 'Disetujui Admin'].includes(String(status || '').trim());
}


async function closePendingLhuScheduleChanges({ idRegistrasi, idJadwalLhu, actorNik = null, transaction }) {
...
function isFinalLhu(row = {}) {
  return String(row?.status_lhu || '').trim() === LHU_STATUS.APPROVED_FINAL;
}

function getSampleLhuRows(sample = {}) {
  const rows = pickArray(sample, ['lhus', 'Lhus', 'LHUList']);

  if (rows.length > 0) {
    return rows;
  }

  const single = pickObject(sample, ['lhu', 'Lhu', 'LHU']);
  return single ? [single] : [];
}

function buildLhuCoverage(sampleRows = []) {
  ...
  sampleRows.forEach((sample) => {
    ...
    const lhuRows = getSampleLhuRows(sample);
    const finalRows = lhuRows.filter(isFinalLhu);
...
async function validateScheduleDateOrThrow(value, label = 'Tanggal pengambilan') {
  const holidays = await loadHolidaysForScheduleValidation();
  return assertScheduleBusinessDateOrThrow(value, label, holidays);
}

async function assertAllLhuFinalForRegistrasi(idRegistrasi, transaction) {
  const requestId = String(idRegistrasi || '').trim();
  ...
  const fpplSampelRows = pickArray(fppl, ['fppl_sampels', 'FpplSampels', 'fppl_sampel', 'FpplSampel']);

  const sampleRows = fpplSampelRows.flatMap((row) => {
    const samples = pickArray(row, ['sampels', 'Sampels', 'sampel', 'Sampel']);
    return samples;
  });
  ...
  const coverage = buildLhuCoverage(sampleRows);

  if (coverage.missingSamples.length > 0) {
    const err = new Error(
      `Masih ada sampel yang belum memiliki LHU: ${coverage.missingSamples.join(', ')}.`
    );
    err.statusCode = 400;
    throw err;
  }
  ...
  if (coverage.notFinalSamples.length > 0) {
    const err = new Error(
      `Masih ada sampel yang LHU-nya belum disahkan Kalab: ${coverage.notFinalSamples.join(', ')}.`
    );
    err.statusCode = 400;
    throw err;
  }
...
async function getPickupQueue() {
  ...
  for (const fpplInstance of fpplInstances) {
    ...
    const schedule = pickObject(fppl, ['jadwal_pengambilan_lhu', 'JadwalPengambilanLhu']) || null;
...
async function schedulePickup({ idRegistrasi, tanggalPengambilan, jamPengambilan, catatan }, currentNik) {
  const requestId = String(idRegistrasi || '').trim();
  ...
  await validateScheduleDateOrThrow(tanggal, 'Tanggal pengambilan LHU');

  const jam = normalizePickupTimeForDb(jamPengambilan);

  const result = await sequelize.transaction(async (transaction) => {
    await assertAllLhuFinalForRegistrasi(requestId, transaction);
    ...
    const payload = {
      ...
      status_pengambilan: 'Disetujui Admin',
  ...
  // Wajib di luar transaksi: simpan jadwal -> commit -> kirim email -> update notifikasi_email
  try {
    await notificationService.notifyJadwalPengambilanLhu(result.id_jadwal_lhu);
  } catch (error) {
...
async function completePickup({ idRegistrasi, namaPengambil }, currentNik) {
  const requestId = String(idRegistrasi || '').trim();
  ...
  return sequelize.transaction(async (transaction) => {
    ...
    if (!isActivePickupScheduleStatus(existing.status_pengambilan)) {
    ...
    await closePendingLhuScheduleChanges({
